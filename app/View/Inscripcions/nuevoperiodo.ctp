          ...
          <div role="alert" class="gritter-item-wrapper with-icon send-o success mb10">
              <div class="gritter-item">
                <div class="gritter-without-image">
                  <span class="gritter-title"><label class="label label-warning">Inscripción</label> en nuevo período académico</span>
                </div>
						...
						<div class="col-sm-2">
							...
							*/
								...
								if (date('m') >= 9 && date('m') <= 12) {
									$periodo =  $year_actual.'-'.($year_actual+1);
								}else{
									$periodo = ($year_actual-1).'-'.$year_actual;
								}
							 ?>
							 <?php echo $this->Form->input('periodo_escolar', array('class'=>'form-control', 'label'=>false, 'value'=>$periodo, 'readonly')); ?>
